...
import type { IntentStatus, IntentSortBy, SortOrder } from "./intents.types";
import {
  Page,
  PageHeader,
  PageContainer,
...
export default function IntentsPage() {
  ...
  const {
    ...
    error,
    refetch,
  } = useIntents({ domainId: defaultDomainId, statusFilter, sortBy, sortOrder, limit });

  const allIntents = data?.pages.flatMap((p) => p.items) ?? [];
  const totalCount = data?.pages[0]?.count ?? 0;

  const handleSort = (field: IntentSortBy) => {
    if (sortBy === field) {
    ...
    <Page>
      ...
      <PageContainer width="list">
        <PageHero
          ...
          icon="🗂️"
          title="Intents"
          description="Proposed intents awaiting signatures and submission."
          badge={{ label: "Intents", note: data ? `${totalCount} total` : undefined }}
        />
