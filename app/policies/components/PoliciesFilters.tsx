"use client";

import { useRef } from "react";
import { useRouter } from "next/navigation";
import type { Core_PolicyScope, Core_Policy } from "custody";

type Core_LockStatus = Core_Policy["lock"];

interface Props {
  scope: Core_PolicyScope | "";
  onScopeChange: (v: Core_PolicyScope | "") => void;
  lock: Core_LockStatus | "";
  onLockChange: (v: Core_LockStatus | "") => void;
  sortBy: string;
  onSortByChange: (v: string) => void;
  domainId?: string;
}

export function PoliciesFilters({
  scope,
  onScopeChange,
  lock,
  onLockChange,
  sortBy,
  onSortByChange,
  domainId,
}: Props) {
  const router = useRouter();
  const idInputRef = useRef<HTMLInputElement>(null);

  const handleIdSearch = (e: React.FormEvent) => {
    e.preventDefault();
    const id = idInputRef.current?.value.trim();
    if (!id) return;
    const params = domainId ? `?domainId=${domainId}` : "";
    router.push(`/policies/${id}${params}`);
  };

  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 space-y-4">
      <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
        Filters
      </h2>
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-1.5">
            Scope
          </label>
          <select
            value={scope}
            onChange={(e) => onScopeChange(e.target.value as Core_PolicyScope | "")}
            className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors bg-white"
          >
            <option value="">All scopes</option>
            <option value="Self">Self</option>
            <option value="Descendants">Descendants</option>
            <option value="SelfAndDescendants">Self & Descendants</option>
          </select>
        </div>

        <div>
          <label className="block text-xs font-medium text-gray-600 mb-1.5">
            Lock
          </label>
          <select
            value={lock}
            onChange={(e) => onLockChange(e.target.value as Core_LockStatus | "")}
            className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors bg-white"
          >
            <option value="">Any lock status</option>
            <option value="Unlocked">Unlocked</option>
            <option value="Locked">Locked</option>
            <option value="Archived">Archived</option>
          </select>
        </div>

        <div>
          <label className="block text-xs font-medium text-gray-600 mb-1.5">
            Sort by
          </label>
          <select
            value={sortBy}
            onChange={(e) => onSortByChange(e.target.value)}
            className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors bg-white"
          >
            <option value="metadata.createdAt">Created At</option>
            <option value="metadata.lastModifiedAt">Last Modified</option>
            <option value="alias">Alias</option>
            <option value="rank">Rank</option>
            <option value="scope">Scope</option>
            <option value="lock">Lock</option>
          </select>
        </div>

        <form onSubmit={handleIdSearch}>
          <label className="block text-xs font-medium text-gray-600 mb-1.5">
            Find by ID
          </label>
          <div className="flex gap-2">
            <input
              ref={idInputRef}
              type="text"
              placeholder="Enter policy ID…"
              className="flex-1 min-w-0 px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors font-mono"
            />
            <button
              type="submit"
              className="px-3 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors flex-shrink-0"
            >
              Go
            </button>
          </div>
        </form>
      </div>
    </div>
  );
}
