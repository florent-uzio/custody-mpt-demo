...
export function ClawbackTab() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="bg-gradient-to-r from-rose-600 to-red-600 rounded-xl shadow-lg p-6 text-white">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h2 className="text-2xl font-bold">Clawback</h2>
        </div>
        <p className="text-rose-100 text-sm">
          Recall issued tokens from a holder back to the issuer. The clawback is
          sent by the <strong>issuer account</strong> and requires the token to
          have been issued as clawback-enabled.
        </p>
        <div className="mt-4 flex items-center gap-2 text-xs">
          <span className="px-2 py-1 bg-white/20 rounded-full">Clawback</span>
          <span className="text-rose-200">XRPL native clawback transaction</span>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <StepCard step={2} title="Currency to claw back">
          ...
          >
            <option value="Currency">Currency (issued token: code + issuer)</option>
            <option value="MultiPurposeToken">MultiPurposeToken (MPT issuance)</option>
        ...
        <StepCard step={3} title="Holder (whose tokens are clawed back)">
          ...
          >
            ...
            <option value="Account">Account (custody account UUID)</option>
            <option value="Endpoint">Endpoint (registered endpoint UUID)</option>
          </select>
          ...
          {holderType === "Account" && (
            <div>
              ...
              <p className="mt-2 text-xs text-gray-500">
                The custody account holding the tokens. Its r-address is shown
                next to the alias.
              </p>
            </div>
          )}
          ...
          {holderType === "Endpoint" && (
            <div>
              ...
              <p className="mt-2 text-xs text-gray-500">
                A registered endpoint (trusted address). Its r-address is shown
                next to the alias.
              </p>
            </div>
          )}
